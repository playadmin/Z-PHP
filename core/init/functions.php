<?php
	...
	function is_index($arr,$r=false){
		...
		return is_numeric($str) ? ($r ? $str == implode('',range(0,count($arr) - 1)) : true) : false;
	}
	...
	*/
	...
	function PDO($c=null,$r=false){
		...
		$dsn = $c['dsn'] ?? 'mysql:host='.$c['DB_HOST'].';dbname='.$c['DB_NAME'].';port='.$c['DB_PORT'];
		$k = md5($dsn);
		if($r || !isset(\z\z::$ZPHP_PDO[$k])){
			...
			\z\z::$ZPHP_PDO[$k] = new \PDO($dsn,$c['DB_USER']??null,$c['DB_PASS']??null,$config);
		}
