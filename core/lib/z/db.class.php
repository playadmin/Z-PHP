<?php
	...
	class db extends sql{
		protected $DB_OBJ,$DB_CONFIG,$DB_CACHE,$DB_CACHEMOD,$DB_QUERYMOD,$DB_MOD,$DOING;
		...
		public function select($field=null,$lock=false){
			$this->DB_MOD && !$this->DOING && $this->DB_QUERYMOD = 1;
		...
		public function update($add,$safe=true){
			$sql = $this->DB_addUpdate($add,$safe);
		...
		public function setInc($field,$num=1){
			$where = $this->DB_sqlWhere();
			$join = $this->DB_JOIN ? ' ' . implode(' ',$this->DB_JOIN) : '';
			$this->DB_SQL = "UPDATE {$this->DB_TABLE}{$join} SET {$field}={$field} + {$num}{$where}";
			return $this->DB_returnNum();
		}
		...
		public function insert($add,$safe=true){
			$sql = $this->DB_addUpdate($add,$safe,true);
			$this->DB_MOD && $this->DB_QUERYMOD = 0;
			$this->DB_SQL = "INSERT INTO {$this->DB_TABLE} {$sql}";
			$starttime = microtime(true);
		...
		public function ifInsert($add,$update=null,$safe=true){
			$add_sql = $this->DB_addUpdate($add,$safe,true);
			$update_sql = $this->DB_addUpdate($update ?: $add,$safe);
			$this->DB_MOD && $this->DB_QUERYMOD = 0;
			$this->DB_SQL = "INSERT INTO {$this->DB_TABLE} {$add_sql} ON DUPLICATE KEY UPDATE {$update_sql}";
			$starttime = microtime(true);
			$num = $this->DB_fetchResult(3);
			$this->DB_sqlend($starttime);
			$this->DB_cl();
		...
		public function getTables(){
			$this->DB_MOD && $this->DB_QUERYMOD = 0;
			switch($this->DB_CONFIG[0]['DB_TYPE']){
				case 'sqlite':
					$this->DB_SQL = "select name from sqlite_master where type='table'";
				break;
				default:
		...
		public function getFields(){
			...
			if(!isset($this->DB_FIELDS[$this->DB_SUBNAME])){
				switch($this->DB_CONFIG[0]['DB_TYPE']){
					case 'sqlite':
						$this->getFieldsFromSqlite();
					break;
					default:
						$this->getFieldsFromMySql();
					break;
				}
			}
			return $this->DB_FIELDS[$this->DB_SUBNAME];
		}
		private function getFieldsFromMySql(){
		...
		private function getFieldsFromSqlite(){
			$this->DB_SQL = "PRAGMA table_info('{$this->DB_PREFIX}{$this->DB_SUBNAME}')";
			$starttime = microtime(true);
			$list = $this->DB_fetchResult(2,\PDO::FETCH_ASSOC,false);
			$this->DB_sqlend($starttime);
			if($list){
				foreach($list as $v){
					$this->DB_FIELDS[$this->DB_SUBNAME][] = $v['name'];
					if('1' === $v['pk']) $this->DB_PRIKEY[$this->DB_SUBNAME][] = $v['name'];
				}
			}
		}
		 ...
		 */
		function rollback(){
			$result = $this->DB_OBJ[$this->DB_QUERYMOD]->rollback();
			$this->DOING = 0;
			return $result;
		}
