...
class db
{
    ...
    public function Select($field = null, $lock = false)
    {
        ...
        if (isset($this->DB_PAGE)) {
            ...
            if (!$lock && $cached = $this->PDO->getCached()) {
                ...
                if (false === $result || !$page = $this->PDO->getCache($pkey, $path)) {
                    $this->DB_page();
                    $sql .= $this->DB_sql(true);
    ...
    private function DB_bindData($data, $act = 'update')
    {
        ...
        foreach ($data as $k => $v) {
            ...
            /** 此段注释下面的代码使用正则存在匹配错误的问题，若想解决此问题，请用此段注释内的代码替换下面的代码 ============ **/
            /** !!!此修改会导致不兼容，所以请自行替换 **/
            /** 使用示例(字段前加冒号表示绑定sql语句)：$data[':inc'] = '`int` + 1' **/
            // $isSqlValue = false;
            // if (is_string($v) && ':' === $k[0]) {
            //     $isSqlValue = true;
            //     $k = substr($k, 1);
            ...
            //     $values[] = $bind_key;
            // }
            /** 此段注释下面的代码使用正则存在匹配错误的问题，若想解决此bug，请用此段注释内的代码替换下面的代码============ **/
